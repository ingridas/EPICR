...
#'
#' @param previous.P - nondominated solutions in the objective space of the solution set obtained 5 evaluations ago
#' @param current.P - nondominated solutions in the objective space of the current solution set
...
check_improvement <- function (previous.front,current.front){
  ...
  previous.HV <- dominated_hypervolume(previous.front, ref)
  current.HV <- dominated_hypervolume(current.front,ref)
